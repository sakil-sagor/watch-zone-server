...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.9clk0.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    const database = client.db("fashion-zone");
    ...
    app.get("/products", async (req, res) => {
      try {
        ...
        const page = parseInt(req.query.page); // default to 0 if not provided
        ...
        const pipeline = [
          ...
          {
            $sort: {
              productName: 1, // Sort by name in ascending order
            },
          },
          { $project: { numericPrice: 0 } }, // Exclude the temporary numeric field from the output
          { $skip: page * size },
          { $limit: size },
        ];
    ...
    app.get("/allproducts", async (req, res) => {
      const allProducts = await productsCollection
        .aggregate([
          { $sample: { size: 100 } }, // You can adjust the size based on your collection's total count
        ])
        .toArray();
    ...
    app.post("/products", async (req, res) => {
      const getLastProd = await productsCollection
        .find({})
        .sort({ _id: -1 })
        .limit(1)
        .toArray();
